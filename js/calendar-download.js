...
// Generate Excel for any month
function generateMonthlyExcel(monthName) {
    try {
        const monthData = CALENDAR_2026_DATA[monthName];
        if (!monthData) {
...
// Universal download function for monthly pages
function downloadMonthlyCalendar(monthName, format = 'pdf') {
    // Show loading overlay
    showLoadingOverlay();
    ...
    setTimeout(() => {
        try {
            ...
            if (format.toLowerCase() === 'pdf') {
                fileName = generateMonthlyPDF(monthName);
            } else if (format.toLowerCase() === 'excel') {
                fileName = generateMonthlyExcel(monthName);
            } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        if (!checkLibrariesLoaded()) {
            console.error('PDF/Excel libraries failed to load');
        }
    }, 1000);
});
